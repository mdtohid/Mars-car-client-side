...
const Header = ({ openModal }) => {
    ...
    const handleMouseEnter = async (value) => {
        setIsFocused(true);
        const products = await productsList.filter(productList => productList.id === value)[0]?.products;
    ...
    return (
        <div onMouseLeave={handleMouseLeave} className={`sticky top-0 z-20 font-serif ${inViewElementId === 'bgBlack' ? 'text-white' : 'text-black'} ${isFocused && 'bg-white !text-black z-30'}`}>
            {/****** Navbar all link *******/}
